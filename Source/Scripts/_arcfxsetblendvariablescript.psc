;this script is a copy of FXSetBlendVariableScript
;that fixes a vanilla bug where it gets stuck in a loop at line 41
;even though the object is unloaded

Scriptname _arcFXSetBlendVariableScript extends ObjectReference  
...
int doWhileLoop 
	...
	;See if we will set blend variable randomly, randomly over time, or once based on input
		...
		while doWhileLoop == 1
			;the following if / else fixes vanilla bug, where this loop will
			;not exit, even though the object is unloaded
			If !Is3DLoaded()
				doWhileLoop = 0
			Else
				myRandom = utility.RandomFloat(myfToggleBlendRageMin,myfToggleBlendRageMax)
